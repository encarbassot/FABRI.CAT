...
// Herramienta makeDOM
// -------------------------------
function makeDOM(queries) {
  const mapQueries = (obj, prefix = '') => {
    const result = {};
    for (const key in obj) {
      const val = obj[key];
      if (typeof val === 'string') {
        result[key] = document.querySelector(val);
      } else if (typeof val === 'object') {
        result[key] = mapQueries(val, `${prefix}${key}.`);
      }
    }
    return result;
  };
  ...
  if (Array.isArray(queries)) {
    ...
    queries.forEach(q => {
      ...
      result[name] = document.querySelector(q);
    });
    return result;
