class ElioCanvas {
  ...
  constructor(width, height, element = undefined) {
      ...
      if(element){
        this.canvas.width = width || element.clientWidth;
        this.canvas.height = height || element.clientHeight;
      }else{
      ...
      if (!element) this.appendTo?.(document.body);

      this._setInitialCSSSize(width, height, element);
      this._applyHiDPI();
      ...
      const onResize = () => {
        if (element) {
          ...
          this.cssWidth = Math.max(1, Math.floor(width || window.innerWidth));
          this.cssHeight = Math.max(1, Math.floor(height || window.innerHeight));
        }
        this._applyHiDPI();
      };
   ...
   // ---- HiDPI helpers ----
   _setInitialCSSSize(width, height, element) {
    if (element) {
      const rect = element.getBoundingClientRect();
      this.cssWidth = Math.max(1, Math.floor(width || rect.width));
      this.cssHeight = Math.max(1, Math.floor(height || rect.height));
    } else {
      this.cssWidth = Math.max(1, Math.floor(width || window.innerWidth));
      this.cssHeight = Math.max(1, Math.floor(height || window.innerHeight));
    }
  ...
  _applyHiDPI() {
    // Limitar DPR extremo si quieres (2 es buen equilibrio rendimiento/nitidez)
    ...
    // Reobtener contexto y escalar el sistema de coordenadas a CSS px
    this.ctx = this.canvas.getContext("2d");
    this.ctx.setTransform(this.dpr, 0, 0, this.dpr, 0, 0);
  }

  resizeToElement(element = this.canvas.parentElement || this.canvas) {
    const rect = element.getBoundingClientRect();
    this.cssWidth = Math.max(1, Math.floor(rect.width));
    this.cssHeight = Math.max(1, Math.floor(rect.height));
    this._applyHiDPI();
